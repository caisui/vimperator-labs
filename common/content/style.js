...
function Highlights(name, store) {
     ...
     */
    this.clear = function () {
        for (let [k, v] of Iterator(highlight))
    ...
    this.loadCSS = function (css) {
        ...
        for (let [class_, hl] of Object.entries(highlight)) {
...
function Styles(name, store) {
    ...
    let cssUri = css => URL.createObjectURL(new Blob([css], {type: "text/css"}));
    ...
    this.__iterator__ = () => Iterator(userSheets.concat(systemSheets));
    this.__defineGetter__("systemSheets", () => Iterator(systemSheets));
    this.__defineGetter__("userSheets",   () => Iterator(userSheets));
    this.__defineGetter__("systemNames",  () => Iterator(systemNames));
    this.__defineGetter__("userNames",    () => Iterator(userNames));
    ...
    this.removeSheet = function (system, name, filter, css, index) {
        ...
        if (filter && filter.indexOf(",") > -1)
            return filter.split(",").reduce(
                (n, f) => n + self.removeSheet(system, name, f, index), 0);
...
Module("styles", {
    ...
    commands: function () {
        ...
        [
            ...
            {
                completer: function (context) { context.completions = styles.sites.map(site => [site, ""]); },
                literal: 1,
    ...
    completion: function () {
        JavaScript.setCompleter(["get", "addSheet", "removeSheet", "findSheets"].map(m => styles[m]),
            [ // Prototype: (system, name, filter, css, index)
                null,
                (context, obj, args) => args[0] ? styles.systemNames : styles.userNames,
                (context, obj, args) => styles.completeSite(context, content),
                null,
                (context, obj, args) => args[0] ? styles.systemSheets : styles.userSheets
            ]);
    }
});
...
Module("highlight", {
    ...
    completion: function () {
        completion.colorScheme = function colorScheme(context) {
            ...
            context.keys = { text: f => f.leafName.replace(/\.vimp$/, ""), description: ".parent.path" };
